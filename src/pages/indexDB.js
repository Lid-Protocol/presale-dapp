...
export default async function DappMetaCache(DappMeta, ProjectName, adding) {
    db.version(1).stores({
        DappMeta:
            '++id,&project,tokenName,tokenSymbol,tokenOwnerWebsite,siteUrl,totalPresale,referralBP,basisPoint,accountCap,favicon,presale,redeemer,timer,token,access,staking'
    });
